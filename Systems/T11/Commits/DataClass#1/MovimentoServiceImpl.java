...
public class MovimentoServiceImpl implements MovimentoService {
	...
	public void salvar(Movimento entidade) {
		if (entidade.getIdMovimento() == null) {
			...
			if (entidade instanceof Lancamento) {
				...
				contrapartida.setConta(((Transferencia) entidade).getContaDestino());
				contrapartida.setContaDestino(entidade.getConta());
	...
	public void editar(Movimento entidade) {
		...
		if (entidade instanceof Lancamento) {
			...
			((Transferencia) entidade).getMovimentoInverso().setConta(((Transferencia) entidade).getContaDestino());
			((Transferencia) ((Transferencia) entidade).getMovimentoInverso()).setContaDestino(entidade.getConta());
	...
	public void validar(Movimento entidade, BindingResult validacao) {
		...
		if (entidade.getConta() != null && entidade instanceof Transferencia
				&& entidade.getConta().equals(((Transferencia) entidade).getContaDestino())) {
